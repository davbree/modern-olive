...
export default class Page extends React.Component {
    render() {
        ...
        return (
            <Layout {...this.props}>
                <section id="main" className={'wrapper' + (this.props.pageContext.frontmatter.sidebar.enabled ? ' sidebar ' + this.props.pageContext.frontmatter.sidebar.side : '')}>
                    <div className="inner">
                        <header className="major">
                            <h2>{_.get(this.props, 'pageContext.frontmatter.title')}</h2>
                            {markdownify(this.props.pageContext.frontmatter.subtitle)}
                        </header>
                        <div className="content">
                            {this.props.pageContext.frontmatter.content_img.enabled && 
                                <Link to={safePrefix(toUrl(this.props.pageContext.pages, this.props.pageContext.frontmatter.content_img.url))} className="image fit"><img src={safePrefix(this.props.pageContext.frontmatter.content_img.path)} alt="" /></Link>
                            }
                            {htmlToReact(_.get(this.props, 'pageContext.html'))}
                        </div>
                        {this.props.pageContext.frontmatter.sidebar.enabled && 
